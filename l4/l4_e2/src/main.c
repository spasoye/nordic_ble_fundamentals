...
#define NOTIFY_SENSOR_INTERVAL_MS 500

static bool app_button_state;
static struct k_work adv_work;
/* STEP 15 - Define the data you want to stream over Bluetooth LE */
static uint32_t simulated_sensor_data = 100;
...
static void simulate_data(void)
{
    ...
    if (simulated_sensor_data == 200) {
    simulated_sensor_data = 100;
    }
}
...
void send_data_thread(void)
{
    while(1){
        ...
        my_lbs_send_sensor_notify(simulated_sensor_data);

        k_sleep(K_MSEC(NOTIFY_SENSOR_INTERVAL_MS));
    }
